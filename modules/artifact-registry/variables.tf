...
variable "keep_recent_count" {
  description = "How many of the most recent versions survive regardless of age, tagged or not. The floor the delete rules cannot cut through, so it bounds how far back a rollback can reach - and only bounds it, since untagged versions count against it too."
  type        = number
  default     = 10

  validation {
    condition     = var.keep_recent_count >= 1
    error_message = "keep_recent_count must be at least 1."
  }
}
